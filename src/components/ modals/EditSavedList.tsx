...
import { updateSavedSearch } from "@/services/auction";
...
const validationSchema = Yup.object({
  savedlist_name: Yup.string().trim().required(ERROR_MESSAGE.NAME_REQUIRED),
});
...
const EditSavedList = (props: ICreateFavList) => {
  ...
  const handleFavlist = (values: { savedlist_name: string }) => {
    const body = {
      name: values?.savedlist_name,
    };
    const payload = {
      body,
      id: fieldata?.id,
      ...
      >
        <div className="w-full">
          <div className="flex flex-col gap-4">
            <CustomFormikForm
              initialValues={{ savedlist_name: fieldata?.name }}
